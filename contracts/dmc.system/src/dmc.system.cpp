    ...
    , _rammarket(_self, _self)
{
    ...
    if (_gstate.total_producer_pst_minted == 0) {
        ...
        for (auto it = idx.cbegin(); it != idx.cend() && 0 < it->total_votes && it->active(); ++it) {
            _gstate.total_producer_pst_minted += it->total_votes;
        }
    }
}
...
void system_contract::bidrefund(account_name bidder, account_name newname)
{
    ...
    INLINE_ACTION_SENDER(eosio::token, transfer)
    (N(eosio.token), { { N(eosio.names), N(active) }, { bidder, N(active) } },
...
EOSIO_ABI(eosiosystem::system_contract,
    ...
    // eosio.system.cpp
    (setram)(setramrate)(setparams)(setpriv)(setalimits)(rmvproducer)
    // eosio.system.cpp
    // (bidname)(bidrefund)
