...
void system_contract::onblock(block_timestamp timestamp, account_name producer)
{
    ...
    if (timestamp.slot - _gstate.last_producer_schedule_update.slot > 120) {
        ...
        (N(eosio.token), { N(eosio), N(active) }, { "allocation" });

        INLINE_ACTION_SENDER(eosio::token, liquidation)
        (N(eosio.token), { N(eosio), N(active) }, { "liquidation" });
...
void system_contract::claimrewards(const account_name& owner)
{
    ...
    if (_gstate.total_producer_pst_minted > 0) {
        producer_per_vote_pay = int64_t((_gstate.pervote_bucket * prod.total_votes) / _gstate.total_producer_pst_minted);
    }
